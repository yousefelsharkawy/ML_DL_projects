...
# mine: enable the TF32 for faster training (reduces the precision of the internals in matmul operation)
torch.set_float32_matmul_precision('high') # enable the TF32
...
use_compile = False # torch.compile interferes with the generation code TODO fix
if use_compile:
...
for step in range(max_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        # for mixed precision training (bfloat16)
        with torch.autocast(device_type=device, dtype=torch.bfloat16):
            logits, loss = model(x,y)
        #logits, loss = model(x,y)
